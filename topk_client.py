...
- Lexical obrigatório com .must(match(...)); para query só numérica, pula BM25
- Boost/resiliência p/ número: (field == num) OR match(num)
...
def _dbg(msg: str) -> None:
    if DEBUG: print(f"[TOPK DEBUG] {msg}")
...
W_CAPUT  = float(os.getenv("TOPK_W_CAPUT",  "0.25"))
W_EMENTA = float(os.getenv("TOPK_W_EMENTA", "0.15"))
...
def _init() -> None:
    ...
    if not _SDK_IMPORTED or Client is None:
        _init_error = "sdk_not_imported"; print("[WARN TOPK] SDK topk_sdk indisponível."); return
    api_key = os.getenv("TOPK_API_KEY"); region  = os.getenv("TOPK_REGION")
    if not api_key or not region:
        _init_error = "missing_env"; print("[WARN TOPK] TOPK_API_KEY/TOPK_REGION ausentes."); return
    try:
        ...
        if _collection is None:
            ...
            print(f"[WARN TOPK] Coleção '{COLLECTION_NAME}' não disponível."); return
        _init_error = None
        if DEBUG: _dbg(f"type(collection)={type(_collection)}")
    except Exception as e:
        _client = None; _collection = None; _init_error = f"init_error:{e}"
...
def _as_dict(rec: Any) -> Dict[str, Any]:
    if isinstance(rec, dict): return rec
...
def _merge_caput_texto(item: Dict[str, Any]) -> str:
    ...
    if caput and texto and texto.startswith(caput): return texto
...
def _normalize_item(raw: Any) -> Dict[str, Any]:
    ...
    excerto = (
        item.get("trecho") or item.get("excerto") or _merge_caput_texto(item) or
        item.get(EMENTA_FIELD) or item.get("text") or item.get("chunk") or
        item.get("content") or ""
    ).strip()
    ...
    return {
        "doc_id": doc_id, "artigo_numero": artigo, "titulo": titulo, "trecho": excerto,
        "score": score, "url": url, "numero_portaria": numero, "ano": ano, "_raw": item,
    }
...
def _extract_number(q: str) -> Optional[str]:
    m = re.search(r"\b(\d{2,6})\b", q); return m.group(1) if m else None
...
def _hybrid_query(col, q: str, k: int) -> List[Dict[str, Any]]:
    ...
    - Se a query for só numérica, não usamos bm25_score() (evita erro do engine).
    """
    if not _QUERY_IMPORTED:
        _dbg("topk_sdk.query indisponível — instale topk-sdk>=0.5.0"); return []
    ...
    try:
        # Monta SELECT (condicionalmente inclui bm25_score)
        if not only_num:
            sel = select(
        ...
        # Lexical obrigatório CONTA para BM25: use .must(...)
        if not only_num:
            qbuilder = qbuilder.must( match(q_norm) | match(q_ascii) )
        ...
        if not only_num:
            ...
            final_score = SEM_WEIGHT*sem_mix  # sem BM25

        qbuilder = qbuilder.topk(final_score, k)

        try: qbuilder = qbuilder.rerank()
...
def search_topk(query: str, k: int = 5) -> List[Dict[str, Any]]:
    ...
    # Fallback extra: semântico puro, mas preservando o número se existir
    ...
    if DEBUG: _dbg(f"search_topk: query='{query}' -> {len(sane)} itens (antes dedupe {len(results)})")
