...
# ============================================================
# NOME CURTO (NORMATIZAÇÃO POR EXCEÇÕES)
# ============================================================
...
def _key_nome(nome: str) -> str:
    nome = nome or ""
    nome = re.sub(r"\(.*?\)", "", nome)         # remove (Mot.) etc
    nome = nome.replace(",", " ")
    nome = re.sub(r"[^\w\s]", " ", nome)        # remove pontuação
    nome = re.sub(r"\s+", " ", nome).strip()
    nome = _strip_accents(nome).upper()
    return nome

_NOME_ALIAS = {
    _key_nome("HERALDO SUDA"): "HERALDO",
    _key_nome("Juliano Mazza Borges"): "Juliano Mazza",
    _key_nome("ANA PAULA WOLF"): "Wolf",
    _key_nome("FERNANDO SANTI VALENTE"): "Fernando",
    _key_nome("THIAGO LIMA MELCO"): "Melco",
    _key_nome("DENIS RODRIGO DA SILVA"): "Rodrigo",
    _key_nome("FÁBIO GULART DE LIMA AGOSTINHAK"): "Fábio",
    _key_nome("ADRIANO ALVES DE SIQUEIRA"): "Siqueira",
    _key_nome("SILVIO ODAIR CUSTÓDIO DOS SANTOS"): "Custódio",
    _key_nome("REGINALDO ANDRÉ RICETO"): "Riceto",
    _key_nome("ROBERTO DA SILVA FRANCO"): "Roberto",
    _key_nome("RODRIGO ARANTES"): "Arantes",
    _key_nome("THIAGO DE LIMA MELCO"): "Melco",
    _key_nome("FABRÍCIO ANDREZER DE LARA"): "Andrezer",
    _key_nome("ZILOMAR BENASSUTE FERNANDES"): "Zilomar",
    _key_nome("Cristiano Alan Kardec de Oliveira"): "Kardec",
    _key_nome("José Nivaldo de Carvalho Junior (Mot.)"): "Carvalho",
    _key_nome("Leandro Patrick Davet"): "Davet",
    _key_nome("Everaldo de Almeida Santos"): "Almeida",
    _key_nome("Luciano de Oliveira Franco"): "Luciano",
    _key_nome("Gabriel dos Santos"): "Gabriel",
    _key_nome("ROMILDO RAMALHO DOS SANTOS"): "Ramalho",
    _key_nome("Adriano Cesar de Lima"): "De Lima",
    _key_nome("Janderson Jucemil Elias"): "Janderson",
    _key_nome("FERNANDO CORDEIRO"): "Fernando",
    _key_nome("JOSÉ SEBASTIÃO NUNES"): "Nunes",
    _key_nome("MARIA JOSÉ SILVA DOS SANTOS"): "Maria",
    _key_nome("IZIDORIO BRUCZKONSKI JUNIOR"): "BRUCZKONSKI",
    _key_nome("JOSIANE PEREIRA DA SILVA FOLADOR"): "JOSIANE",
    _key_nome("MARIA FERNANDA NUNES BARBOSA"): "Fernanda",
}

def normatizar_nome_curto(nome: str) -> str:
    k = _key_nome(nome)
    if k in _NOME_ALIAS:
        return _NOME_ALIAS[k]

    # fallback: primeiro nome (preserva acentos)
...
def encurtar_responsavel(resp: str) -> str:
    """
    Mantém posto/grad e reduz apenas o NOME, usando normatizar_nome_curto.
    Ex.: "3º Sgt. QP PM Adriano Cesar de Lima" -> "3º Sgt. QP PM De Lima"
    """
    resp = (resp or "").strip()
    if not resp:
        return ""
    m = re.search(r"\bPM\b\s+(.*)$", resp, flags=re.IGNORECASE)
    if m:
        nome = m.group(1).strip()
        prefixo = resp[:m.start(1)]
        return f"{prefixo}{normatizar_nome_curto(nome)}"
    return normatizar_nome_curto(resp)
# ============================================================
# EXTRAIR DATA
# ============================================================
...
def extrair_extrajornada(caminho_pdf: str):
    ...
    - Cada escala inicia em uma linha "HORÁRIO:" e termina no próximo "HORÁRIO:" ou na ASSINATURA / início de outra parte do boletim.
    - VTRs: conta somente VTRs do tipo 1xxxx ou Lxxxx (mesma lógica do extrair_corp), deduplicadas por escala.
    - Efetivo: cada policial = (posto/grad + nome). Conta por linhas de tabela (com tolerância a quebra de linha).
    - Assinatura: ao encontrar, fecha a escala atual para não "contaminar" com texto administrativo.
    """
    import pdfplumber
    import re
    ...
    # Início de escala por turno
    padrao_horario_linha = re.compile(r"^\s*HOR[ÁA]RIO\s*:\s*(.+)$", re.IGNORECASE)
    ...
    # Aceita: "3º Sgt. QP PM", "Sgt.", "Sd QP PM", etc.
    padrao_posto_grad = re.compile(
        r"\b(?:(\d+)[º°]?\s*)?(Ten\.?|Sgt\.?|Cb\.?|Sd\.?)\.?(?:\s+(?:QP|QOEM))?(?:\s+PM)?\b",
        re.IGNORECASE
    )
    ...
    def limpar_linha_tabela(linha: str) -> str:
        # remove colunas iniciais "EQ" e "VTR" se vierem como números
        s = linha.strip()
    ...
    def extrair_posto_nome(linha: str):
        ...
        # posto_grad = trecho exato encontrado, padronizando espaços
        ...
        if not nome or len(nome.split()) < 2:
            # se não deu para extrair nome confiável, ainda assim retorna algo
            return posto_grad, nome
    ...
    with pdfplumber.open(caminho_pdf) as pdf:
        for pagina in pdf.pages:
            ...
            for linha in texto.split("\n"):
                ...
                # fim do bloco extra (ou início de outra parte do boletim)
                ...
                if not dentro_tabela:
                    # ainda assim, captura telefone do responsável como fallback (caso a tabela esteja quebrada)
                ...
                if pendente and (not linha_tem_posto(linha)) and tem_rg_ou_tel(linha):
                    ...
                    # telefone do responsável (primeiro policial da tabela)
                ...
                if linha_tem_posto(linha):
                    ...
                    # se já tem RG/tel na mesma linha, fecha o policial imediatamente
                    if tem_rg_ou_tel(linha):
                        ...
                        # pode estar quebrado na próxima linha
                        pendente = {"posto_grad": posto_grad, "nome": nome or ""}
                        # se ainda não tem responsável, setamos com o que temos (completa quando vier linha de RG/tel)
                        if not escala_atual["responsavel"]:
                            escala_atual["responsavel"] = f"{posto_grad} {nome}".strip()
